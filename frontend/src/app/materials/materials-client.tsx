...
export default function MaterialsClient() {
  ...
  const handleAskRagAgent = (docTitle: string) => {
    const targetUrl = `/chat?q=Explain+key+concepts+from+${encodeURIComponent(docTitle)}`;
  ...
  const handleDirectFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      await uploadDocument(file, selectedCategory === 'All' ? 'Polity' : selectedCategory);
    } catch (err) {
      console.log('Document upload notice:', err);
    }
    ...
    setUploadSuccessToast(cleanTitle);
  };
    ...
    <div className="flex min-h-screen bg-[#f4f1ea] text-[#19241d]">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <main className="p-6 lg:p-8 space-y-6 overflow-y-auto max-w-6xl mx-auto w-full">
          ...
          {/* Success Toast Banner with Instant Ask RAG Agent CTA */}
          {uploadSuccessToast && (
            <div className="bg-emerald-50 border border-emerald-300 text-emerald-900 p-4 rounded-xl text-xs font-bold flex flex-col md:flex-row md:items-center justify-between gap-3 shadow-sm animate-in fade-in">
              <div className="flex items-center gap-2">
                <Sparkles className="w-4 h-4 text-emerald-700 shrink-0" />
                <span>✅ Document <strong>"{uploadSuccessToast}"</strong> successfully chunked & stored in Supabase pgvector!</span>
              </div>
              <div className="flex items-center gap-2">
                <button
                  onClick={() => handleAskRagAgent(uploadSuccessToast)}
                ...
                <button
                  ...
                  className="p-1 text-emerald-700 hover:text-emerald-950 font-bold ml-2 cursor-pointer"
                >
                  ✕
                </button>
              </div>
                  ...
                  <div className="mt-5 pt-3 border-t border-[#f0ece1] flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      <button
                        ...
                        className="px-3 py-1.5 rounded-lg bg-[#22352a] text-white hover:bg-[#2e4739] text-xs font-bold transition-colors shadow-sm cursor-pointer flex items-center gap-1.5"
                      >
